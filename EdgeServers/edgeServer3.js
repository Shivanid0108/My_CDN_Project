...
const { getCachedContent, cacheContent, deleteCachedContent } = require('./edgeDatabase3');
...
app.get('/content/:filename', async (req, res) => {
  ...
  try {
    ...
    console.error(`[Edge Server 3] Error: ${error.message}`);
...
app.delete('/content/:filename', async (req, res) => {
  ...
  try {
    ...
    console.error(`[Edge Server 3] Error deleting cached content: ${error.message}`);
    res.status(500).send('Error deleting cached content');
  }
});

http2.createServer(serverOptions, app).listen(PORT, () => {
  console.log(`Edge Server 3 running on HTTP/2 and HTTPS at port ${PORT}`);
});
